  ...
  private
    ...
    def set_poll
      ...
      @poll = Rails.cache.fetch key, expires_in: 10.minutes, race_condition_ttl: 20.seconds do
        if params[:poll]
          @poll = Poll.with_associations.find_by!(slug: params[:poll])
        elsif params[:poll_id]
          @poll = Poll.with_associations.find(params[:poll_id])
        else
          @poll = Poll.with_associations.joins(:domains).find_by!(domains: {domain: request.host})
        end
      end
    end
end
